...
function ExerciseStatsPage() {
  ...
  async function load() {
    setLoading(true);
    const [exData, setsData, mesoData, bwData] = await Promise.all([
      offlineRead<Exercise>(
        () => supabase.from("exercises").select("*").eq("id", exerciseId).maybeSingle(),
        async () => (offlineDB ? (await offlineDB.exercises.get(exerciseId)) ?? null : null)
      ),
      ...
      offlineReadList<any>(
        ...
        async () => (offlineDB ? offlineDB.mesocycles.orderBy("start_date").toArray() : null)
      ),
      offlineRead<{ bodyweight_kg: number }>(
        () =>
          supabase
        ...
        async () => {
          ...
          const list = await offlineDB.workout_sessions.filter((s) => s.bodyweight_kg != null).toArray();
          list.sort((a, b) => b.session_date.localeCompare(a.session_date));
          return list[0] ? { bodyweight_kg: list[0].bodyweight_kg as number } : null;
        }
      ),
    ]);
    ...
    setLatestBodyweight(bwData?.bodyweight_kg ?? null);
    const enriched: SetWithE1RM[] = setsData.map((s) => ({
      ...s,

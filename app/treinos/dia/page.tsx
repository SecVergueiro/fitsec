...
// Rota estática lendo os ids da query — ver comentário em /sessao/ativa.
export default function DiaDetailRoute() {
  return (
...
function DiaDetailPage() {
  ...
  async function loadData() {
    setLoading(true);
    const [dayData, exData] = await Promise.all([
      offlineRead<TemplateDay>(
        () => supabase.from("template_days").select("*").eq("id", dayId).maybeSingle(),
        async () => (offlineDB ? (await offlineDB.template_days.get(dayId)) ?? null : null)
      ),
      offlineReadList<ExerciseWithDetails>(
        () =>
          supabase
        ...
        async () => {
          ...
          const rows = await offlineDB.template_exercises
            .where("template_day_id").equals(dayId)
            .sortBy("exercise_order");
          return Promise.all(
            rows.map(async (r) => ({
              ...r,
              exercise: (await offlineDB.exercises.get(r.exercise_id)) ?? null,
            }))
          ) as Promise<ExerciseWithDetails[]>;
        }
      ),
    ]);
    setDay(dayData);
  ...
  async function reorderExercises(orderedIds: string[]) {
    ...
    await Promise.all(
      reordered.map((e) =>
        offlineUpdate("template_exercises", { exercise_order: e.exercise_order }, { id: e.id }, {
  ...
  async function deleteExercise(id: string) {
    ...
    if (!ok) return;
    await offlineDelete("template_exercises", { id }, { localTable: "template_exercises", localId: id });
    loadData();
  }
  ...
  async function deleteDay() {
    ...
    if (!ok) return;
    await offlineDelete("template_days", { id: dayId }, { localTable: "template_days", localId: dayId });
    router.push(templateId ? `/treinos/template?id=${templateId}` : "/treinos");
  }
  ...
  if (!day) {
    return (
      <div className="fade-in">
        <Link
          href={`/treinos/template?id=${templateId}`}
        ...
        <EmptyState
          ...
          action={
            <Link href={`/treinos/template?id=${templateId}`}>
...
function EditPrescriptionModal({
  ...
  async function save() {
    ...
    await offlineUpdate(
      ...
      { localTable: "template_exercises", localId: templateExercise.id }
    );
    setSaving(false);
    onSaved();
  }

...
// Rota estática lendo o id da query string.
//
// Antes isto era /sessao/[id], uma rota dinâmica (ƒ no build): offline não
// existe HTML pra servir e o service worker caía no offline.html — ou seja,
// iniciar um treino sem internet levava a um beco sem saída. Como /sessao/ativa
// é prerenderizada, ela é pré-cacheada e funciona sem rede para qualquer id.
export default function SessaoAtivaRoute() {
  return (
...
function SessaoAtivaPage() {
  ...
  async function load() {
    ...
    const [sessionData, mesoData, exData] = await Promise.all([
      ...
      offlineRead<{ start_date: string; total_weeks: number }>(
        ...
        async () => {
          ...
          // is_active é boolean e o IndexedDB não aceita boolean como chave —
    ...
    const enriched = await Promise.all(
      (exData as any[]).map(async (ex) => {
        const sets = await offlineReadList<SessionSet>(
          () => supabase.from("session_sets").select("*").eq("session_exercise_id", ex.id).order("set_number"),
          async () =>
            offlineDB
  ...
  async function toggleSuperset(idx: number) {
    ...
    if (ex.superset_group != null) {
      ...
      await offlineUpdate("session_exercises", { superset_group: null }, { id: ex.id }, { localTable: "session_exercises", localId: ex.id });
      setExercises((cur) => {
        const next = [...cur];
    ...
    if (group == null) {
      ...
      await offlineUpdate("session_exercises", { superset_group: group }, { id: prev.id }, { localTable: "session_exercises", localId: prev.id });
    }
    await offlineUpdate("session_exercises", { superset_group: group }, { id: ex.id }, { localTable: "session_exercises", localId: ex.id });
...
function ExerciseCard({
  ...
  async function savePrescription() {
    await offlineUpdate(
      ...
      { id: exercise.id },
      { localTable: "session_exercises", localId: exercise.id }
    );
